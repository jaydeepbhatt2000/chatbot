...
{
 //   std::cout<<"find match called "<<std::endl;
    response_list.clear();
//std::cout<<"find match called "<<std::endl;
    for(size_t  i = 0; i < nKnowledgeBaseSize; ++i)
    {
  // std::cout<<"i="<<i<<std::endl;
   // std::cout<<"knowledgebase=  "<<KnowledgeBase<<std::endl;
     // std::cout<<"knowledgebase keyword=  "<<KnowledgeBase[i].keyword<<std::endl;
     //std::cout<<"knowledgebase response=  "<<*KnowledgeBase[i].response<<std::endl;

        std::string keyWord(KnowledgeBase[i].keyword);
        // there has been some improvements made in
        // here in orde to make the matching process
...
void CBot:: copy(const char *array[], vstring &v2) {
   // std::cout<<"copy function called "<<std::endl;
   ...
   //  std::cout<<"copy function khatam "<<std::endl;
}


void cleanString(std::string &str) {
   // std::cout<<"cleanstring called "<<std::endl;
    int len = str.length();
    std::string temp = "";
...
void UpperCase(std::string &str) {
 ...
 //   std::cout<<"uppercase called "<<std::endl;
    int len = str.length();
    for(int i=0; i<len; i++){
        if(str[i]>= 'a' && str[i]<= 'z') {
        str[i] -= 'a'-'A';
...
void copy_new(const char *array_new[], vstring &v_new) {
       for(int i=0;i<MAX_RESP;++i) {
       v_new.push_back(array_new[i]);
      }
}
